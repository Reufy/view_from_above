...
def photo_search_vk(coefficient):
    def result_photo(photo_name_f, first_photo_dig_f, second_photo_load_f):
        ...
        if a <= coefficient:
            if a <= 0.30:
                ...
                a = 1 - coefficient
            a = str(a)
            res_name = a + "% совпадение  c фотографией пользователя vk.com/id" + photo_name_f[2:-6]
            return res_name
    ...
    try:
        with shelve.open('index_base_photo_vk/index_base_photo_vk', 'r') as photo_vk:
            if photo_vk:
                for unloading_name, unloading_data in photo_vk.items():
                ...
                for x in arr_result:
                    print(x)
                print("Колличество результатов " + str(len(x)) + "; Время поиска: %s сек" % (time.time() - start_time))
            else:
                print("В базе данных отсутствуют изображения")
                pause_exit()
